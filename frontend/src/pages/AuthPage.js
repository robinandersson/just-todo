...
class AuthPage extends Component {
  ...
  handleSubmit = (evt) => {
    ...
    fetch('http://localhost:8000/graphql', {
      ...
      if (resData.errors) throw new AuthError(resData.errors[0].message, resData.errors[0].code);


      if (resData.data.login.token) {
        const { userId, username: loginUsername, token, tokenExpiration } = resData.data.login;
  ...
  render() {
    if (this.context.token) return <Redirect to={ this.props.authedRedirectTo } />;
      ...
      <React.Fragment>
        ...
        >
          ...
          <div>
            <button
              type="submit"
              className={"btn mt-5 mb-2 py-1 px-3" + (!this.state.isLoginPath ? " mode--positive" : "") }
            >
