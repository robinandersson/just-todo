...
class PreferencesPage extends Component {
  ...
  componentWillMount() {
    ...
    fetch(`${SERVER_URL}${GRAPHQL_ROUTE}`, {
      ...
      if (res.status !== 200 && res.status !== 200) throw new Error('Fetching user data failed!');
  ...
  isReadyToSubmit = () => !!(this.state.newPassword && (this.state.newPassword === this.state.repeatPassword));
  ...
  handleUserPrefUpdate = (evt) => {
    ...
    fetch(`${SERVER_URL}${GRAPHQL_ROUTE}`, {
      ...
      if (res.status !== 200 && res.status !== 201) throw new Error('Failed to update user preferences!');
      return res.json();
    }).then(resData => {
      if (resData.errors) throw new Error('Failed to update user preferences!');
    }).catch(err => {
      console.log(err);
      return err;
  ...
  render() {
    ...
    return (
      <form className="container form" onSubmit={ this.handleUserPrefUpdate }>
        ...
        <label className="input-label" htmlFor="repeat-password">Repeat Password</label>
        <input
          className="text-input tracking-widest"
          name="repeatPassword"
          type="password"
        ...
        <div className="mt-5 mb-2 flex items-center">
          ...
          <div className={ "transition" + (!this.state.isLoading ? " scale" : "") }>
            <FontAwesomeIcon
              icon={ faCog }
              className={ "ml-2 text-green-400 transition mode--instant spin" + (!this.state.isLoading ? " opacity-100" : " opacity-0") }
              size="2x"
            />
