...
const createTodoMutation = {
  ...
  resolve(parent, args, req) {
    const query = 'INSERT INTO todos(description, is_completed, created_at, completed_at, user_id) VALUES ($1, $2, $3, $4, $5) RETURNING *';
    const values = [
      args.description,
      false,
...
const toggleTodoMutation = {
  ...
  resolve(parent, args, req) {
    const query = 'UPDATE todos SET is_completed = NOT is_completed WHERE id = $1';
    return db.none(query, args.id)
      .then(res => res)
      .catch(err => {
        console.log(err);
        return err;
...
const modifyTodoDescriptionMutation = {
  ...
  resolve(parent, args, req) {
    ...
    return db.none(query, [ args.id, args.description ])
      .then(res => res)
      .catch(err => {
        console.log(err);
        return err;
